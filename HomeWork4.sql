...
VALUES 
        ...
        ("Vicont",3,1),
        ("Zuza",4,3);
/*1.Вывести всех котиков по магазинам по id (условие соединения 
...
WHERE name = "Murzik";
 ...
 SELECT shops.*, m_cat.*
  ...
  FROM cats 
  WHERE name = "Murzik")  AS m_cat
  ON shops.id  = m_cat.shops_id;
  
  /*Вывести магазины, в которых НЕ продаются коты 
  “Мурзик” и “Zuza”*/
  SELECT shops.*, cats.*
FROM shops
JOIN cats 
...
WHERE name != "Murzik" AND name != "Zuza";
 ...
 SELECT shops.*, new_cat.*
  FROM shops
  JOIN
  (SELECT name, shops_id
  FROM cats 
  WHERE name != "Murzik" AND name != "Zuza")  AS new_cat
  ON shops.id  = new_cat.shops_id;
...
JOIN orders 
ON an_id = ord_an
ORDER BY orders.ord_datetime ASC
LIMIT 4,25;